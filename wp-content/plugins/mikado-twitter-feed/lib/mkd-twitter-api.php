...
class MikadoTwitterApi {
	...
	public function obtainRequestToken() {
		...
		if(is_wp_error($response)) {
			...
			$responseObj->message = __('Internal WP error', 'mkd-twitter-feed');
		} else {
				...
				if((is_array($responseParsed) && count($responseParsed)) &&
				   !empty($responseParsed['oauth_token']) && !empty($responseParsed['oauth_token_secret'])
					...
					if(!empty($responseObj->redirectUrl)) {
						$responseObj->status  = false;
						$responseObj->message = __('Redirect URL couldn\t not be generated', 'mkd-twitter-feed');
					} else {
					...
					$responseObj->status  = false;
					$responseObj->message = __('Couldn\'t connect with Twitter API', 'mkd-twitter-feed');
				}
			}
		}
			...
			if(is_wp_error($response)) {
				...
				if(is_array($responseParsed) && count($responseParsed)
				   && !empty($responseParsed['oauth_token']) && !empty($responseParsed['oauth_token_secret']) && !empty($responseParsed['user_id']) && !empty($responseParsed['screen_name'])
					...
					$responseObj->status  = true;
					$responseObj->message = __('Access token obtained', 'mkd-twitter-feed');
				}
			}
		} else {
			$responseObj->status  = false;
			$responseObj->message = __('Authorize token and it\'s secret were not obtainer', 'mkd-twitter-feed');
		}
	...
	public function fetchTweets($userId = '', $count = '', $transient = array()) {
		...
		if(!$this->transientExists($transient)) {
			if($userId && $accessToken) {
				...
				if(is_wp_error($response)) {
					...
					if(isset($response['response']['code']) && $response['response']['code'] == 200) {
						...
						$responseObj->status  = false;
						$responseObj->message = __('Couldn\'t connect with Twitter', 'mkd-twitter-feed');
					}
				}
			} else {
				$responseObj->status  = false;
				$responseObj->message = __('It seams like you haven\t connected with your Twitter account', 'mkd-twitter-feed');
			}
		} else {
			...
			if(!empty($transientContent)) {
				...
				$responseObj->status  = false;
				$responseObj->message = __('Couldn\'t retreive content from database', 'mkd-twitter-feed');
			}
		}
