...
<div class="panel-body panel-white">
 <ul class="nav nav-tabs panel_tabs" role="tablist">
      <li class="<?php if($active_tab == 'memberlist') echo "active";?>">
          <a href="?dashboard=user&page=alumni&tab=memberlist">
             <i class="fa fa-align-justify"></i> <?php _e('Alumni List', 'gym_mgt'); ?></a>
          </a>
      </li>
	<?php if($obj_gym->role == 'staff_member'){?>
	<?php 
	...
	{?>
	 <li class="<?php if($active_tab == 'viewmember') echo "active";?>">
      	<a href="?dashboard=user&page=alumni&tab=addmember">
        <i class="fa fa-plus-circle"></i> <?php		
			_e('View Alumni', 'gym_mgt'); 		
		?></a> 
      </li>
	<?php }
	  }} ?>
		 	...
		 	foreach ($membersdata as $retrieved_data)
			{
               ...
               <td class="status"><?php echo $retrieved_data->membership_status;?></td>
				...
				if($obj_gym->role == 'staff_member' || ($obj_gym->role == 'member' && $retrieved_data->ID==$curr_user_id)){
					?>
				<a class="btn btn-success" href="?dashboard=user&page=alumni&tab=viewmember&action=view&member_id=<?php echo $retrieved_data->ID;?>"><?php _e('View','gym_mgt');?></a>
				<a href="?dashboard=user&page=alumni&tab=add_attendence&member_id=<?php echo $retrieved_data->ID;?>&attendance=1" class="btn btn-default"  idtest="<?php echo $retrieved_data->ID; ?>"><i class="fa fa-eye"></i> <?php _e('View Attendance','gym_mgt');?> </a>
				<?php }?>
				</td>
            </tr>
				<?php }
