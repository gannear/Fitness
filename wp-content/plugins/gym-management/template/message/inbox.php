			...
			foreach($message as $msg)
			{
				...
				<tr>
					<td><?php echo gym_get_display_name($msg->sender);//echo get_user_name_byid($msg->sender);?></td>
					 <td>
						 <a href="?dashboard=user&page=message&tab=view_message&from=inbox&id=<?php echo $msg->message_id;?>"> <?php echo $msg->subject;if($obj_message->gmgt_count_reply_item($msg->post_id)>=1){?><span class="badge badge-success pull-right"><?php echo $obj_message->gmgt_count_reply_item($msg->post_id);?></span><?php } ?></a>
					</td>
					<td><?php echo wp_trim_words($msg->message_body,5)?>
					</td>
					<td>
						<?php  echo  getdate_in_input_box($msg->date);?>
					</td>
				</tr>
				<?php 
			}
