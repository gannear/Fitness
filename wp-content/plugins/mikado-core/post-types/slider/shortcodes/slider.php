...
class Slider implements Lib\ShortcodeInterface {
	...
	public function render($atts, $content = null) {
		...
		if($slider != "") {
			...
			if($topfit_options['smooth_page_transitions'] == "yes" && $topfit_options['smooth_pt_spinner_type'] != "") {
				$ajax_loader = '<div class="mkd-st-loader"><div class="mkd-st-loader1">'.topfit_mikado_loading_spinners(true).'</div></div>';
			} else {
				$ajax_loader = '<div class="mkd-st-loader"><div class="mkd-st-loader1">'.topfit_mikado_loading_spinner_pulse().'</div></div>';
			}

			/* set slider preloader - end */

			/* set padding for slider arrows - start */
			$slider_arrows_padding = "padding-top: ".esc_attr($this->topfit_mikado_get_slider_navigation_padding())."px;";
			/* set padding for slider arrows - end */

			$html .= '<div id="mkd-'.esc_attr($slider).'" '.$slider_anchor.' '.$responsiveness_data.' '.$responsive_coefficients_graphic_data.' '.$responsive_coefficients_title_data.' '.$responsive_coefficients_subtitle_data.' '.$responsive_coefficients_text_data.' '.$responsive_coefficients_button_data.' class="carousel slide '.esc_attr($animation_type_class.' '.$full_screen_class.' '.$responsive_height_class.' '.$height_class.' '.$auto_start_class.' '.$header_effect_class.' '.$slider_numbers_class.' '.$slider_thumbs_class).' " '.$slide_animation_timeout.' '.$data_height.' '.$data_parallax_effect.' '.topfit_mikado_get_inline_style($slide_holder_height).'><div class="mkd-slider-preloader">'.$ajax_loader.'</div>';
			$html .= '<div class="carousel-inner '.esc_attr($slider_css_position_class).'" '.topfit_mikado_get_inline_style($carouselinner_height).' '.$data_parallax_transform.'>';
			...
			if(have_posts()) : $postCount = 0;
				while(have_posts()) : the_post();
					...
					$thumbnail_attributes        = topfit_mikado_get_attachment_meta_from_url($thumbnail, array(
						'width',
						'height'
					));
					...
					if(get_post_meta(get_the_ID(), "mkd_slide_content_vertical_middle_type", true) == 'window_top') {
						...
						$slide_item_padding_value = $this->topfit_mikado_get_slider_item_padding();
					}
					...
					if($slide_type == 'video') {
						...
						$html .= '<div class="mkd-video"><div class="mkd-mobile-video-image" '.topfit_mikado_get_inline_style('background-image: url('.esc_url($video_image).')').'></div><div class="mkd-video-overlay';
						...
						$html .= '<div class="mkd-image" '.topfit_mikado_get_inline_style('background-image:url('.esc_url($image).')').'>';
						if($slider_thumbs == 'no') {
							$html .= '<img src="'.esc_url($image).'" alt="'.esc_attr($title).'">';
						}

						if($image_overlay_pattern !== "") {
							$html .= '<div class="mkd-image-pattern" '.topfit_mikado_get_inline_style('background: url('.esc_url($image_overlay_pattern).') repeat 0 0').'></div>';
						}
						$html .= '</div>';
					}
					...
					if(($meta_temp_subtitle = get_post_meta(get_the_ID(), "mkd_slide_subtitle", true)) != "") {
						...
						if(($meta_temp = get_post_meta(get_the_ID(), 'mkd_slide_subtitle_background_color', true)) !== '') {
							...
							$slide_subtitle_span_style_array[] = 'background-color: '.esc_attr(topfit_mikado_rgba_color($slide_subtitle_bg_color, $slide_subtitle_bg_transparency)).'';
						}
						...
						//topfit_mikado_inline_style($slide_subtitle_style_array); var_dump($slide_subtitle_style_array); die();
						$html_text .= '<h3 class="mkd-slide-subtitle" '.topfit_mikado_get_inline_style($slide_subtitle_style_array).'><span '.topfit_mikado_get_inline_style($slide_subtitle_span_style_array).'>'.wp_kses_post($meta_temp_subtitle).'</span></h3>';
						$html_text .= '</div>';
					}
					...
					if(($meta_temp = get_post_meta(get_the_ID(), 'mkd_slide_title_background_color', true)) !== '') {
						...
						$slide_title_span_style_array[] = 'background-color: '.esc_attr(topfit_mikado_rgba_color($slide_title_bg_color, $slide_title_bg_transparency)).'';
					}
					...
					if(get_post_meta(get_the_ID(), "mkd_slide_hide_title", true) == 'no') {
						$html_text .= '<div class="mkd-el">';
						$html_text .= '<h2 class="mkd-slide-title" '.topfit_mikado_get_inline_style($slide_title_style_array).'>';
						...
						$html_text .= '<span '.topfit_mikado_get_inline_style($slide_title_span_style_array).'>'.wp_kses_post($title).'</span>';
						if($meta_temp != '') {
							$html_text .= '</a>';
						}
					...
					if(($meta_temp = get_post_meta(get_the_ID(), 'mkd_slide_text_background_color', true)) !== '') {
						...
						$slide_text_span_style_array[] = 'background-color: '.esc_attr(topfit_mikado_rgba_color($slide_text_bg_color, $slide_text_bg_transparency)).'';
					}
					...
					if(($meta_temp = get_post_meta(get_the_ID(), "mkd_slide_text", true)) != "") {
						$html_text .= '<div class="mkd-el">';
						$html_text .= '<h3 class="mkd-slide-text" '.topfit_mikado_get_inline_style($slide_text_style_array).'><span '.topfit_mikado_get_inline_style($slide_text_span_style_array).'>'.wp_kses_post($meta_temp).'</span></h3>';
						$html_text .= '</div>';
					}
					...
					if($is_first_button_shown) {
						...
						if($meta_temp_b1_icon_pack != '') {
							...
							$iconPackName                = topfit_mikado_icon_collections()->getIconCollectionParamNameByKey($button1_params['icon_pack']);
						...
						$html_text .= topfit_mikado_get_button_html($button1_params);
					}
					...
					if($is_second_button_shown) {
						...
						if($meta_temp_b2_icon_pack != '') {
							...
							$iconPackName                = topfit_mikado_icon_collections()->getIconCollectionParamNameByKey($button2_params['icon_pack']);
							if(($meta_temp = get_post_meta(get_the_ID(), 'button2_icon_'.$iconPackName, true)) != '') {
								$button2_params[$iconPackName] = $meta_temp;
							}
						}
						//var_dump($button1_params); die();
						$html_text .= topfit_mikado_get_button_html($button2_params);
					}
					...
					if($separate_text_graphic != 'yes') {
						$html .= '<div class="mkd-slider-content '.esc_attr($content_alignment).'" '.topfit_mikado_get_inline_style($content_width.$content_xaxis.$content_yaxis_start).' '.$slide_data_start.' '.$slide_data_end.'>';
						if(($meta_temp = get_post_meta(get_the_ID(), "mkd_slide_content_vertical_middle", true)) == "yes") {
							$html .= '<div class="mkd-slider-content-inner '.esc_attr($content_animation.' '.$content_animation_direction).'" '.topfit_mikado_get_inline_style($vertical_content_width.$vertical_content_xaxis).'>';
						}
						$html .= $html_thumb;
						$html .= $html_text;
						if($meta_temp == "yes") {
						...
						$html .= '<div class="mkd-slider-content mkd-graphic-content '.esc_attr($graphic_alignment).'" '.topfit_mikado_get_inline_style($graphic_width.$graphic_xaxis.$graphic_yaxis_start).' '.$graphic_data_start.' '.$graphic_data_end.'>';
						if(($meta_temp = get_post_meta(get_the_ID(), "mkd_slide_content_vertical_middle", true)) == "yes") {
							$html .= '<div class="mkd-slider-content-inner" '.topfit_mikado_get_inline_style($vertical_content_width.$vertical_content_xaxis).'>';
						}
						$html .= $html_thumb;
						if($meta_temp == "yes") {
							$html .= '</div>';
	 ...
	 **/
	function topfit_mikado_get_slider_item_padding() {
		return apply_filters('topfit_mikado_slider_item_padding', 0);
	}

	/**
	 * Function that returns slider navigation pading
	 **/
	function topfit_mikado_get_slider_navigation_padding() {
		return apply_filters('topfit_mikado_slider_navigation_padding', 0);
	}
}