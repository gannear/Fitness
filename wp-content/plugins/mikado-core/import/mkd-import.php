...
class TopFitMikadoImport {
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'mkd_options_topfit', $options);
    ...
    function mkd_admin_import() {
        if (mkd_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                'topfit_mikado_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                array($topfit_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'topfit_mikado_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'topfit_mikado_enqueue_admin_styles');
...
function mkd_init_import_object(){
    global $topfit_import_object;
    $topfit_import_object = new TopFitMikadoImport();
}

add_action('init', 'mkd_init_import_object');

if(!function_exists('topfit_mikado_dataImport')){
    function topfit_mikado_dataImport(){
        global $topfit_import_object;

        if ($_POST['import_attachments'] == 1)
            $topfit_import_object->attachments = true;
        else
            $topfit_import_object->attachments = false;

        $folder = "topfit/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $topfit_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_mkd_dataImport', 'topfit_mikado_dataImport');
}

if(!function_exists('topfit_mikado_widgetsImport')){
    function topfit_mikado_widgetsImport(){
        global $topfit_import_object;
        ...
        $topfit_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_mkd_widgetsImport', 'topfit_mikado_widgetsImport');
}

if(!function_exists('topfit_mikado_optionsImport')){
    function topfit_mikado_optionsImport(){
        global $topfit_import_object;
        ...
        $topfit_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_mkd_optionsImport', 'topfit_mikado_optionsImport');
}

if(!function_exists('topfit_mikado_otherImport')){
    function topfit_mikado_otherImport(){
        global $topfit_import_object;
        ...
        $topfit_import_object->import_options($folder.'options.txt');
        $topfit_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $topfit_import_object->import_menus($folder.'menus.txt');
        $topfit_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_mkd_otherImport', 'topfit_mikado_otherImport');
}