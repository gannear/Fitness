...
if (!function_exists('mkd_core_activation')) {
	 ...
	 * and defines topfit_mikado_core_on_activate action
	 */
	function mkd_core_activation() {
		do_action('topfit_mikado_core_on_activate');
...
if (!function_exists('mkd_core_themename_theme_menu')) {
	...
	function mkd_core_themename_theme_menu() {
		if (mkd_core_theme_installed()) {
			...
			topfit_mikado_init_theme_options();
			...
			$page_hook_suffix = add_menu_page(
				...
				'topfit_mikado_theme_menu',                // The ID used to bind submenu items to this menu
				array($topfit_Framework->getSkin(), 'renderOptions'), // The callback function used to render this menu
				$topfit_Framework->getSkin()->getMenuIcon('options'),             // Icon For menu Item
				$topfit_Framework->getSkin()->getMenuItemPosition('options')            // Position
			);

			foreach ($topfit_Framework->mkdOptions->adminPages as $key => $value) {
				$slug = "";
				...
				$subpage_hook_suffix = add_submenu_page(
					...
					array($topfit_Framework->getSkin(), 'renderOptions')
				);

				add_action('admin_print_scripts-' . $subpage_hook_suffix, 'topfit_mikado_enqueue_admin_scripts');
				add_action('admin_print_styles-' . $subpage_hook_suffix, 'topfit_mikado_enqueue_admin_styles');
			};

			add_action('admin_print_scripts-' . $page_hook_suffix, 'topfit_mikado_enqueue_admin_scripts');
			add_action('admin_print_styles-' . $page_hook_suffix, 'topfit_mikado_enqueue_admin_styles');
