...
class EonetAdmin extends Eonet
{
    ...
    public function ajaxSaveSettings() {
        ...
        if($value || $eonet_settings == get_option($option_field)) {
            ...
            $response['title'] = esc_html__('Done, everything is safe !', 'eonet-manual-user-approve');
            $response['content'] = esc_html__('Your settings have been saved successfully.', 'eonet-manual-user-approve');
        } else {
            $response['status'] = 'error';
            $response['title'] = esc_html__('Something went wrong...', 'eonet-manual-user-approve');
            $response['content'] = esc_html__("We've not been able to save your settings, please try again later.", 'eonet-manual-user-approve');
        }
    ...
    public function ajaxResetSettings() {
        ...
        if($value) {
            $response['status'] = 'success';
            $response['title'] = esc_html__('Done, settings have been reset !', 'eonet-manual-user-approve');
            $response['content'] = esc_html__('The page will be refreshed in a few seconds...', 'eonet-manual-user-approve');
        } else {
            $response['status'] = 'error';
            $response['title'] = esc_html__('Something went wrong...', 'eonet-manual-user-approve');
            $response['content'] = esc_html__("We've not been able to reset your settings, please try again later.", 'eonet-manual-user-approve');
        }
    ...
    public function adminMenus(){
	    ...
	    add_menu_page(
		    esc_html__( 'Eonet Dashboard', 'eonet-manual-user-approve' ),
		    esc_html__( 'Eonet','eonet-manual-user-approve' ),
		    'manage_options',
		    'eonet',
    ...
    static public function getThemes() {
        $themes = array(
            array(
                'name' => esc_html__('Woffice Intranet/Extranet theme', 'eonet-manual-user-approve'),
                ...
                'premium' => true,
                'features' => array(
                    esc_html__('Projects Management','eonet-manual-user-approve'),
                    esc_html__('Complete Wiki/Knowledge base','eonet-manual-user-approve'),
                    esc_html__('Members map & directory','eonet-manual-user-approve'),
                    esc_html__('Responsive layout (mobile & tablet)','eonet-manual-user-approve'),
                    esc_html__('Cover picture','eonet-manual-user-approve'),
                    esc_html__('Private / Public pages','eonet-manual-user-approve'),
                    esc_html__('Files Manager','eonet-manual-user-approve'),
                    esc_html__('Events calendar','eonet-manual-user-approve'),
                )
            ),

	        array(
		        'name' => esc_html__('Eonet - Multi-Purpose Community / Network WordPress Theme', 'eonet-manual-user-approve'),
		        ...
		        'premium' => true,
		        'features' => array(
			        esc_html__('Designed for communities','eonet-manual-user-approve'),
			        esc_html__('1-to-1 live chat','eonet-manual-user-approve'),
			        esc_html__('Super fast loading','eonet-manual-user-approve'),
			        esc_html__('Restrict pages and contents','eonet-manual-user-approve'),
			        esc_html__('Fully customizable','eonet-manual-user-approve'),
			        esc_html__('Members map','eonet-manual-user-approve'),
			        esc_html__('Social login','eonet-manual-user-approve'),
			        esc_html__('Files Manager','eonet-manual-user-approve'),
		        )
	        )
        );
