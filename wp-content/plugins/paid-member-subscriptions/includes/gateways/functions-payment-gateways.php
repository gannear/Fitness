...
function pms_get_output_payment_gateways( $pms_settings = array() ) {
    ...
    // If there's only one payment gateway saved
        ...
        $output .= apply_filters( 'pms_output_payment_gateway_input_hidden', '<input type="hidden" class="pms_pay_gate" name="pay_gate" value="' . esc_attr( $paygate_key ) . '" ' . $gateway_supports_arr[$paygate_key] . ' />', $active_gateways[0] );
