...
class AppPresser_AP3_Extension_Compatibility {
	...
	?>
		<aside class="io-modal" id="geo-checkin-form" tabindex="-1" role="dialog" aria-hidden="true">
			...
			<?php $appcheckin_anonymous = appp_get_setting('appcheckin_anonymous', false); ?>
			<?php if ( $appcheckin_anonymous || is_user_logged_in() ) : ?>
				<?php $user_ID = ( is_user_logged_in() ) ? get_current_user_id() : 0; ?>
	 ...
	 *
	 * To filter the child theme slug, use the 'appp_theme' hook.
	 *
	 * @return string Path to template
	 */
	public function appbuddy_new_login() {

		if( file_exists( get_stylesheet_directory() . '/buddypress/appbuddy-login.php' ) ) {

			// child theme
			$template = get_stylesheet_directory() . '/buddypress/appbuddy-login.php';
