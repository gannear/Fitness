	...
	function login_modal_template() {
		...
		<aside class="io-modal" id="loginModal" tabindex="-1" role="dialog" aria-hidden="true">
			<div class="bar bar-header bar-white">
				<div class="title"><?php _e( 'Login', 'ap3-ion-theme' ) ?></div>
			...
			<div class="io-modal-content has-header">
				...
				<?php appp_login_modal_before();
					...
					if ( !is_user_logged_in() ) {
						...
						/**
						 * The redirect filter here is almost irrelevant
						 * because the login ajax response sets
						 * the redirect_url too.  Not 100% sure it's even needed.
						 */
						$redirect = apply_filters( 'appp_login_redirect', '' );
						if( $redirect && is_string( $redirect ) ) {
							wp_login_form(array(
								'redirect' => $redirect,
							));
						} else if( $redirect && is_array( $redirect ) && isset( $redirect['url'] ) && $redirect['url'] ) {
							wp_login_form(array(
						...
						echo '<p><a class="button button-secondary ajaxlogout" href="' . wp_logout_url( get_permalink() ) . '">'.__('Logout', 'ap3-ion-theme') .'</a></p>';
					}
	...
	function appp_lost_password_template() {
		...
		<aside class="io-modal" id="app-lost-password" tabindex="-1" role="dialog" aria-hidden="true">
			...
			<div class="io-modal-content">
				...
				<p><input type="email" id="lost_email" name="email" value="" placeholder="<?php _e( 'Email', 'ap3-ion-theme' ); ?>"/></p>
				...
				<p><input type="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" id="reset-code" name="reset-code" value="" placeholder="<?php _e( 'Code', 'ap3-ion-theme' ); ?>"/></p>
				<p><input type="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" id="app-pw" name="app-pw" value="" placeholder="<?php _e( 'New Password', 'ap3-ion-theme' ); ?>"/></p>
				<p><input type="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" id="app-pwr" name="app-pwr" value="" placeholder="<?php _e( 'Repeat Password', 'ap3-ion-theme' ); ?>"/></p>
