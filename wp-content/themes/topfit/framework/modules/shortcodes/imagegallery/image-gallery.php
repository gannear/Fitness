<?php
namespace TopFit\Modules\Shortcodes\ImageGallery;
...
class ImageGallery implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		vc_map(array(
			'name'                      => esc_html__('Image Gallery', 'topfit'),
			'base'                      => $this->base,
			'category'                  => 'by MIKADO',
			...
			'params'                    => array(
				...
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__('Image Size', 'topfit'),
					'param_name'  => 'image_size',
					'description' => esc_html__('Enter image size. Example: thumbnail, medium, large, full or other sizes defined by current theme. Alternatively enter image size in pixels: 200x100 (Width x Height). Leave empty to use "thumbnail" size', 'topfit')
				),
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Gallery Type', 'topfit'),
					'admin_label' => true,
					'param_name'  => 'type',
					'value'       => array(
						esc_html__('Slider', 'topfit')     => 'slider',
						esc_html__('Image Grid', 'topfit') => 'image_grid'
					),
					'description' => esc_html__('Select gallery type', 'topfit'),
					'save_always' => true
				),
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Show Title & Description', 'topfit'),
				...
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Slide duration', 'topfit'),
					'admin_label' => true,
					'param_name'  => 'autoplay',
					'value'       => array(
						'3'                                  => '3',
						'5'                                  => '5',
						'10'                                 => '10',
						esc_html__('Disable', 'topfit') => 'disable'
					),
					'save_always' => true,
					'dependency'  => array(
						'element' => 'type',
					...
					'description' => esc_html__('Auto rotate slides each X seconds', 'topfit'),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Column Number', 'topfit'),
				...
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Spacing', 'topfit'),
					'param_name'  => 'spacing',
					'value'       => array(
						esc_html__('Yes', 'topfit') => 'yes',
						esc_html__('No', 'topfit')  => 'no'
				...
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Open PrettyPhoto on click', 'topfit'),
				...
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Grayscale Images', 'topfit'),
				...
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Enable Circle Overlay Hover', 'topfit'),
					...
					'admin_label' => true,
					'save_always' => true,
					'description' => esc_html__('Default value is No', 'topfit'),
					'dependency'  => array(
				...
				array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Show Navigation Arrows', 'topfit'),
					'param_name'  => 'arrows',
					'value'       => array(
						esc_html__('Yes', 'topfit') => 'yes',
						esc_html__('No', 'topfit')  => 'no'
	...
	public function render($atts, $content = null) {
		...
		$html = topfit_mikado_get_shortcode_module_template_part('templates/' . $template, 'imagegallery', '', $params);
