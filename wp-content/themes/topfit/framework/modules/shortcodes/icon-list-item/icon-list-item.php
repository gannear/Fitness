<?php
namespace TopFit\Modules\IconListItem;
...
class IconListItem implements ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(array(
			'name'     => esc_html__('Icon List Item', 'topfit'),
			'base'     => $this->base,
			...
			'params'   => array_merge(
				...
				array(
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Title Font Weight', 'topfit'),
					...
					array(
						...
						'value'       => array(
							esc_html__('Default', 'topfit')              => 'default-font-family',
							esc_html__('Headings Font Family', 'topfit') => 'headings-font-family'
						),
						'save_always' => true,
						'description' => '',
						'dependency'  => array(
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Spacing between Title and Icon (px)', 'topfit'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Margin Bottom (px)', 'topfit'),
	...
	public function render($atts, $content = null) {
		...
		if ($params['margin_bottom'] !== '') {
			$params['holder_styles'][] = 'margin-bottom: ' . topfit_mikado_filter_px($params['margin_bottom']) . 'px';
		}
		...
		$html = topfit_mikado_get_shortcode_module_template_part('templates/icon-list-item-template', 'icon-list-item', '', $params);
	...
	private function getIconStyle($params) {
		...
		if (!empty($params['icon_size'])) {
			$iconStylesArray[] = 'font-size:' . topfit_mikado_filter_px($params['icon_size']) . 'px';
		}
	...
	private function getTitleSubtitleStyle($params) {
		...
		if (!empty($params['space_title_and_icon'])) {
			$styles[] = 'padding-left: ' . topfit_mikado_filter_px($params['space_title_and_icon']) . 'px';
		}
	...
	private function getTitleStyle($params) {
		...
		if (!empty($params['title_size'])) {
			$titleStylesArray[] = 'font-size:' . topfit_mikado_filter_px($params['title_size']) . 'px';
		}
