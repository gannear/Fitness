<?php
namespace TopFit\Modules\Shortcodes\Icon;

use TopFit\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package TopFit\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
			...
			'params'                    => array_merge(
				...
				array(
					...
					array(
						'type'        => 'textfield',
						'admin_label' => true,
						'heading'     => esc_html__('Custom Size (px)', 'topfit'),
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Shadow', 'topfit'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Border radius', 'topfit'),
						'param_name'  => 'border_radius',
						'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'topfit'),
						'dependency'  => array(
							'element' => 'type',
							'value' => array('square')
						)
					...
					array(
						...
						'admin_label' => true,
						'description' => esc_html__('Enter just number. Omit pixels', 'topfit'),
						'dependency'  => array(
							'element' => 'type',
							'value' => array('circle', 'square')
						)
					...
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_html__('Hover Icon Color', 'topfit'),
					...
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_html__('Hover Border Color', 'topfit'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Margin', 'topfit'),
						'param_name'  => 'margin',
						'admin_label' => true,
						'description' => esc_html__('Margin (top right bottom left)', 'topfit')
					),
					array(
						'type'        => 'dropdown',
					...
					array(
						'type'        => 'checkbox',
						'heading'     => esc_html__('Use Link as Anchor', 'topfit'),
						'value'       => array(esc_html__('Use this icon as Anchor?', 'topfit') => 'yes'),
						'param_name'  => 'anchor_icon',
						'admin_label' => true,
						'description' => esc_html__('Check this box to use icon as anchor link (eg. #contact)', 'topfit'),
						'dependency'  => array(
							'element' => 'link',
							'not_empty' => true
						)
	...
	public function render($atts, $content = null) {
		...
		$html = topfit_mikado_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
	...
	private function generateIconStyles($params) {
		...
		if (($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
			...
			if (!empty($params['custom_size'])) {
				$iconStyles[] = 'font-size:' . topfit_mikado_filter_px($params['custom_size']) . 'px';
			}
		}
	...
	private function generateIconHolderStyles($params) {
		...
		if (isset($params['type']) && $params['type'] !== 'normal') {
			...
			if (!empty($shapeSize)) {
				$iconHolderStyles[] = 'width: ' . topfit_mikado_filter_px($shapeSize) . 'px';
				$iconHolderStyles[] = 'height: ' . topfit_mikado_filter_px($shapeSize) . 'px';
				$iconHolderStyles[] = 'line-height: ' . topfit_mikado_filter_px($shapeSize) . 'px';
			}
			...
			if (!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: ' . topfit_mikado_filter_px($params['border_width']) . 'px';
			...
			if ($params['type'] == 'square') {
				if (isset($params['border_radius']) && $params['border_radius'] !== '') {
					$iconHolderStyles[] = 'border-radius: ' . topfit_mikado_filter_px($params['border_radius']) . 'px';
				}
			}
		}
