<?php
namespace TopFit\Modules\CallToAction;
...
class CallToAction implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		$call_to_action_button_IconCollections = topfit_mikado_icon_collections()->iconCollections;
		foreach ($call_to_action_button_IconCollections as $collection_key => $collection) {

			$call_to_action_button_icons_array[] = array(
				'type'        => 'dropdown',
				'heading'     => esc_html__('Button Icon', 'topfit'),
		...
		vc_map(array(
			'name'                      => esc_html__('Call to Action', 'topfit'),
			'base'                      => $this->getBase(),
			'category'                  => 'by MIKADO',
			...
			'params'                    => array_merge(
				array(
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Full Width', 'topfit'),
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Content in grid', 'topfit'),
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Grid size', 'topfit'),
				...
				topfit_mikado_icon_collections()->getVCParamsArray(array(
					'element' => 'type',
				...
				array(
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Icon Size (px)', 'topfit'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Box Padding (top right bottom left) px', 'topfit'),
						'param_name'  => 'box_padding',
						'admin_label' => true,
						'description' => esc_html__('Default padding is 20px on all sides', 'topfit'),
						'group'       => esc_html__('Design Options', 'topfit')
					),
					array(
						'type'        => 'colorpicker',
					...
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_html__('Box Border Color', 'topfit'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Box Border Width (px)', 'topfit'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Default Text Font Size (px)', 'topfit'),
						'param_name'  => 'text_size',
						'description' => esc_html__('Font size for p tag', 'topfit'),
						'group'       => esc_html__('Design Options', 'topfit')
					),
					array(
						'type'        => 'dropdown',
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Button Type', 'topfit'),
						'param_name'  => 'button_type',
						'value'       => array_flip(topfit_mikado_get_btn_types(true)),
						'save_always' => true,
						'admin_label' => true,
						'dependency'  => array(
							'element' => 'show_button',
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Hover Button Type', 'topfit'),
					...
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_html__('Button Color', 'topfit'),
					...
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_html__('Button Hover Color', 'topfit'),
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Button Hover Animation', 'topfit'),
						'param_name'  => 'button_hover_animation',
						'value'       => array_flip(topfit_mikado_get_btn_hover_animation_types(true)),
						'save_always' => true,
						'admin_label' => true,
						'dependency'  => array(
							'element' => 'show_button',
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Button Position', 'topfit'),
						'param_name'  => 'button_position',
						'value'       => array(
							esc_html__('Default/right', 'topfit') => '',
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Button Size', 'topfit'),
						...
						'value'       => array(
							...
							esc_html__('Extra Large', 'topfit') => 'big_large'
						),
						'description' => '',
						'dependency'  => array(
							'element' => 'show_button',
					...
					array(
						...
						'admin_label' => true,
						'description' => esc_html__('Default text is "button"', 'topfit'),
						'dependency'  => array(
							'element' => 'show_button',
							'value' => array('yes')
						)
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Button Target', 'topfit'),
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Button Icon Pack', 'topfit'),
						'param_name'  => 'button_icon_pack',
						'value'       => array_merge(array('No Icon' => ''), topfit_mikado_icon_collections()->getIconCollectionsVC()),
						'save_always' => true,
						'dependency'  => array(
							'element' => 'show_button',
							'value' => array('yes')
	...
	public function render($atts, $content = null) {
		...
		$args = array_merge($args, topfit_mikado_icon_collections()->getShortcodeParams(), $call_to_action_icons_form_fields);
		...
		$html = topfit_mikado_get_shortcode_module_template_part('templates/call-to-action-template', 'calltoaction', '', $params);
	...
	private function getCallToActionStyles($params) {
		...
		if ($params['box_border_color'] != '') {
			...
			if ($params['box_border_width'] !== '') {
				$border_width = topfit_mikado_filter_px($params['box_border_width']);
			}
	...
	private function getCallToActionIcon($params) {
		...
		$icon = topfit_mikado_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
		$iconStyles = array();
		$iconStyles['icon_attributes']['style'] = $this->getIconStyles($params);
		$call_to_action_icon = '';
		if (!empty($params[$icon])) {
			$call_to_action_icon = topfit_mikado_icon_collections()->renderIcon($params[$icon], $params['icon_pack'], $iconStyles);
		}
	...
	private function getButtonParameters($params) {
		...
		if (!empty($params['button_icon_pack'])) {
			...
			$iconPackName = topfit_mikado_icon_collections()->getIconCollectionParamNameByKey($params['button_icon_pack']);
