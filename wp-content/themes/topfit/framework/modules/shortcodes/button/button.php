<?php
namespace TopFit\Modules\Shortcodes\Button;
 ...
 * @package TopFit\Modules\Shortcodes\Button
 */
class Button implements ShortcodeInterface {
		...
		vc_map(array(
			...
			'params'                    => array_merge(
				array(
					...
					array(
						...
						'value'       => array(
							...
							esc_html__('Extra Large', 'topfit')            => 'huge',
							esc_html__('Extra Large Full Width', 'topfit') => 'huge-full-width'
						),
						'dependency'  => array(
							'element' => 'type',
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Hover Type', 'topfit'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Custom CSS class', 'topfit'),
				...
				array(
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Hover Animation', 'topfit'),
						'param_name'  => 'hover_animation',
						'group'       => esc_html__('Hover Animation', 'topfit'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Icon Font Size (px)', 'topfit'),
					...
					array(
						...
						'description' => esc_html__('Insert margin in format: 0px 0px 0px 0px', 'topfit'),
						'admin_label' => true,
						'group'       => esc_html__('Design Options', 'topfit')
					),
					array(
					...
					array(
						...
						'param_name'  => 'margin',
						'description' => esc_html__('Insert margin in format: 0px 0px 1px 0px', 'topfit'),
						'admin_label' => true,
						'group'       => esc_html__('Design Options', 'topfit')
					)
				)
	...
	public function render($atts, $content = null) {
		...
		return topfit_mikado_get_shortcode_module_template_part('templates/' . $params['html_type'], 'button', '', $params);
	}
	...
	private function getHoverAnimation($params) {
		...
		return topfit_mikado_options()->getOptionValue('button_hover_animation');
	}
	...
	private function getIconHolderStyles($params){
		...
		if($params['icon_font_size']!=''){
			$styles = 'font-size: ' . topfit_mikado_filter_px($params['icon_font_size']) . 'px;';
		}
