					...
					<a class="mkd-product-thumbnail-link" href="<?php the_permalink(); ?>"
					   title="<?php the_title_attribute(); ?>">
						...
						<?php if (get_post_meta($product->get_id(), 'mkd_single_product_new_meta', true) === 'yes') : ?>
							<span class="mkd-new-product">
								<?php esc_html_e('NEW', 'topfit'); ?>
							</span>
						<?php endif;?>
						<?php if (!$product->is_in_stock()) : ?>
							<span class="mkd-out-of-stock">
								<?php esc_html_e('SOLD', 'topfit'); ?>
							</span>
						<?php endif; ?>
						<?php if ($product->is_on_sale()) : ?>
							<span class="mkd-out-of-stock">
								<?php esc_html_e('SALE', 'topfit'); ?>
							</span>
						<?php endif; ?>
					</a>
							...
							<?php if ($rating_enabled) { ?>
								<div class="mkd-pl-rating-holder">
									<div class="star-rating"
										 title="<?php printf(esc_html__('Rated %s out of 5', 'topfit'), $average); ?>">
