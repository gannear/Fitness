<?php
namespace TopFit\Modules\PieCharts\PieChartDoughnut;
...
class PieChartDoughnut implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ($i = 1; $i <= $this->chartFields; $i++) {
			...
			$chart_fields[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__('Chart Value', 'topfit') . $i,
				'param_name' => "chart_value_" . $i,
			);
			$chart_fields[] = array(
				'type'       => 'colorpicker',
				'heading'    => esc_html__('Chart Color', 'topfit') . $i,
				'param_name' => "chart_color_" . $i,
			);
			$chart_fields[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__('Chart Legend', 'topfit') . $i,
		...
		vc_map(array(
			'name'                      => esc_html__('Pie Chart 3 (Doughnut)', 'topfit'),
			'base'                      => $this->getBase(),
			'category'                  => 'by MIKADO',
			...
			'params'                    => array_merge(
				array(
					...
					array(
						'type'       => 'textfield',
						'heading'    => esc_html__('Height', 'topfit'),
						'param_name' => 'height',
					),
				),
				$chart_fields
	...
	public function render($atts, $content = null) {
		...
		$html = topfit_mikado_get_shortcode_module_template_part('templates/pie-chart-doughnut', 'piecharts/piechartdoughnut', '', $params);
