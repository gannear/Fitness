<?php
namespace TopFit\Modules\Header\Lib;
...
class HeaderConnector {
	...
	public function connect($config = array()) {
		do_action('topfit_mikado_pre_header_connect');
		...
		if(!empty($config['affect_content'])) {
			add_filter('topfit_mikado_content_elem_style_attr', array($this, 'contentMarginFilter'));
		}

		if(!empty($config['affect_title'])) {
			add_filter('topfit_mikado_title_content_padding', array($this, 'titlePaddingFilter'));
		}

		if(!empty($config['affect_slider'])) {

			add_filter('topfit_mikado_slider_item_padding', array($this, 'sliderItemPaddingFilter'));
			add_filter('topfit_mikado_slider_navigation_padding', array(
				$this,
				'sliderNavigationPaddingFilter'
			));
		}

		do_action('topfit_mikado_after_header_connect');
	}
	 ...
	 *
	 * Hooks to topfit_mikado_title_content_padding filter
	 *
	 * @return int
	 */
	 ...
	 *
	 * Hooks to topfit_mikado_slider_item_padding filter
	 *
	 * @return int
	 */
